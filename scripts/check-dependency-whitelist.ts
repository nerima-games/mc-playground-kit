 ...
 *   repository declares locally. This copy carries the FULL 16-repository
 *   roster of plan.md §2.1, so any cycle in the declared architecture is
 *   visible from here. The cost is that the roster is duplicated 16 times and
 *   kept in sync by hand; the roster should eventually be published once from
 *   mc-dev-meta and consumed, at which point this comment goes away.
 * - The graph is a declaration, not an observation. It says what the
 *   architecture permits, not what the sibling repositories' package.json files
 *   actually contain. A sibling that adds an undeclared edge is caught by ITS
 *   OWN copy of this gate (its `dependencies` would fail `undeclared-in-policy`),
 *   not by this one.
 */
 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-playground-kit the authoritative row is
 *   mc-worldgen, mc-sim, mc-render   (+ mc-kernel, universally)
 *
 * This repository is the one named in DEV_ONLY_PACKAGES below, so the script
 * running here is also the script that defines the rule constraining every
 * consumer of this package: mc-playground-kit may appear only in a sibling's
 * `devDependencies`, never in `dependencies`.
 *
 * That asymmetry is deliberate and worth restating. The constraint is on who
 * may depend on *this* package. This package's own dependencies are ordinary
 * runtime dependencies — a playground genuinely constructs a world, a
 * simulation and a renderer at run time. What must never happen is a shipped
 * module reaching back through this harness for a service (input above all),
 * because the harness is not in the release build.
 * ---------------------------------------------------------------------------
 */
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-playground-kit',

  dependencyGraph: new Map<string, ReadonlySet<string>>([
    // The full 16-repository roster of plan.md §2.1, recorded in every copy so
    // that cycle detection can actually see a cycle. `@nerima-games/mc-kernel`
    // is deliberately absent from every row: it is universally importable
    // (rule 4) and listing it is a policy-config error.
    //
    // Tier 1 — stable libraries. Mutually independent; buildable in parallel.
    ...
    // Tier 2 — foundations (nouns). State and services.
    ...
    // Tier 3 — experience modules (verbs). Zero edges between them, by design:
    // they talk only through the foundation services (plan.md §2.3-1).
    //
    // mx-gameplay and mx-redstone additionally take @nerima-games/mc-playground-kit
    // as a devDependency. That is NOT a runtime edge and so does not appear
    // here — DEV_ONLY_PACKAGES below is what enforces it, and a dev-only package
    ...
    // Tier 4 — composition. Layer merge + the single total stage order + E2E.
    [
    ...
    // Meta — the dev workspace (plan.md §6 Step 0-2). It clones the other
    // repositories into `repos/`; it depends on none of them.
